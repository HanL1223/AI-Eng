...
def build_pipeline(chunk_size:int = CHUNK_SIZE,overlap:int = CHUNK_OVERLAP):
    ...
    #Step 2:Chunk with experimental chunk_size
    all_chunks = []
    ...
    #Step 4 extract known table name
...
def run_chunk_size_experiment(
    ...
    for i, size in enumerate(chunk_sizes):
        ...
        #Build pipeline with this chunk size
        ...
        if collection is None:
            print(f"SKIPPED: Pipeline build failked for chunk_size ={size}")
            continue
        ...
        print(f"  → Average keyword score: {avg:.2f}")
